...
namespace Lucifer
{
    [AddComponentMenu("Lucifer/GameManager")]
    public sealed class GameManager : MonoBehaviour
    {
        ...
        [SerializeField] private System[] m_Systems     = new System[0];
        ...
        public static TSystem GetSystem<TSystem>() where TSystem : System
        {
            for (int i = 0, length = s_Instance.m_Systems.Length; i < length; i++)
            {
                if (s_Instance.m_Systems[i] is TSystem result)
                {
                    return result;
                }
            }

            throw new ArgumentException($"{typeof(TSystem).Name} is not exists, please setup it on GameManager.");
        }
        ...
        private void Update()
        {
            for (int i = 0, length = m_Systems.Length; i < length; i++)
            {
                m_Systems[i].OnUpdate(m_Entities);
            }
        }
        ...
        private void FixedUpdate()
        {
            for (int i = 0, length = m_Systems.Length; i < length; i++)
            {
                m_Systems[i].OnFixedUpdate(m_Entities);
            }
        }
        ...
        private void LateUpdate()
        {
            for (int i = 0, length = m_Systems.Length; i < length; i++)
            {
                m_Systems[i].OnLateUpdate(m_Entities);
            }
